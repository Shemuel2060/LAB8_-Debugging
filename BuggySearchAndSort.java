...
public class BuggySearchAndSort {
	boolean yes = false; // assume array does not contain val
...
//		return value;
		return false;    // a bug here.....Would always be false
	...
	public static void bubbleSort(int[] array) {
		for (int i = 0; i < array.length; i++) {
			for (int j = 0; j < array.length-1; i++) { // another bug - j should be the one to increment
	...
	public static void selectionSort(int[] array) {
		for (int top = array.length - 1; top > 0; top--) {
			int positionOfMax = 0;
			for (int i = 1; i <= top; i++) {
				if (array[1] > array[positionOfMax]) // a bug here instead of 1 use i....
					positionOfMax = i;
			}
